...
export const GlobalAudioPlayer = () => {
  ...
  // Если текущий трек не выбран, берем первый элемент из очереди
  ...
  useEffect(() => {
    if (audioRef.current && activeTrack) {
      ...
      // Запоминаем событие и восстанавливаем тайминг
      const savedTime = getTrackProgress(activeTrack.id)
      const handleMetadata = () => {
        if (audioRef.current && savedTime > 0 && savedTime < audioRef.current.duration) {
  ...
  const containerStyle: React.CSSProperties = {
    ...
    overflow: 'hidden', // Чтобы скругления обрезали прогресс-бар по углам
  }

  return (
    <div style={{ 
      ...containerStyle, 
      // ИСПРАВЛЕНО: В мини-режиме обнуляем нижний паддинг, чтобы прижать линию к самому краю
      paddingBottom: isPlayerMinimized ? '0px' : 'calc(env(safe-area-inset-bottom, 16px) + 10px)',
      paddingTop: isPlayerMinimized ? '10px' : '10px',
      paddingLeft: '20px',
      paddingRight: '20px',
      ...
      {isPlayerMinimized ? (
        <div style={{ 
          ...
          alignItems: 'center',
          // Добавляем отступ для безрамочных экранов снизу, но до прогресс-бара
          ...
          {/* ИСПРАВЛЕНО: Линия прогресса перемещена в самый низ мини-плеера */}
