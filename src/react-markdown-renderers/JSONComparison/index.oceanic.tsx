...
//   display: flex;
//   align-items: center; /* Оптимизация 1: Центрируем текст, чтобы он не упирался в границы строки */
//   width: max-content;
//   min-width: 100%;
//   box-sizing: border-box;
//   height: 24px;
...
//   /* Оптимизация 2: Убираем любые скроллбары для строки. */
//   /* overflow-y: hidden гарантирует, что вертикальный скролл больше не появится */
//   overflow-x: visible; 
...
//   & span.punct,
//   & span.str,
//   & span.num-bool { font-family: ui-monospace, SFMono-Regular, SF Mono, Menlo, Consolas, monospace, Courier !important; }
// `
...
const ListWrapper = styled.div`
  ...
  /* Оптимизация 1: Возвращаем скролл на уровень контейнера, но теперь он работает правильно */
  overflow-x: auto; 
  overflow-y: hidden;
...
const CodeLine = styled.div<ICodeLineProps>`
  display: flex;
  align-items: center;
  /* Оптимизация 2: Строка занимает всю ширину самого длинного контента */
  width: max-content;
  min-width: 100%;
  box-sizing: border-box;
  height: 24px;
...
const Sign = styled.div<ICodeLineProps>`
  ...
  /* Оптимизация 3: Закрепляем знаки слева железно! */
  position: sticky;
  left: 0;
  /* Фон под знаком должен совпадать с фоном строки, чтобы текст JSON под него не просвечивал */
  background-color: ${(props: ICodeLineProps) => {
    if (props.$commaOnly) return '#393d35'; // Смесь фона янтарного и основного
    if (props.$type === 'added') return '#263d33'; // Смесь фона зеленого и основного
    if (props.$type === 'removed') return '#3c292f'; // Смесь фона красного и основного
    return '#263238'; // Чистый фон для неизмененных строк
  }};
...
const Content = styled.div`
  ...
  white-space: pre;
  line-height: 24px;
  padding-left: 8px; /* Небольшой отступ от зафиксированного знака */
  padding-right: 16px;
  font-family: monospace, system-ui !important;
  color: #b0bec5 !important;
  
  /* Оптимизация 4: Никаких внутренних скроллов и вылетов по вертикали */
  overflow: visible;
  ...
  & span.punct,
  & span.str,
  & span.num-bool { font-weight: bold; font-family: ui-monospace, SFMono-Regular, SF Mono, Menlo, Consolas, monospace, Courier !important; }
`
...
export const JSONComparison = ({ json1, json2, height = 500 }: TProps) => {
  ...
  const highlightJsonLine = (lineText: string) => {
    ...
    return <span style={{ fontFamily: 'ui-monospace, SFMono-Regular, SF Mono, Menlo, Consolas, monospace, Courier' }} dangerouslySetInnerHTML={{ __html: escaped }} />;
  }
    ...
    if (!VirtualList) {
      return (
        <FallbackScrollContainer $height={height}>
          {diffResult.map((_, index) => (
            <Row key={index} index={index} style={{ fontFamily: 'ui-monospace, SFMono-Regular, SF Mono, Menlo, Consolas, monospace, Courier' }} />
          ))}
