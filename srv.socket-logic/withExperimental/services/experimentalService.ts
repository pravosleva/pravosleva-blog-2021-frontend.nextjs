import { NEvent, TGeoIpInfo } from '~/srv.socket-logic/withExperimental/types'
import {
  getChannelName, mws,
  // state,
} from '~/srv.socket-logic/withExperimental/utils'
      ...
      if (e.ok) {
        ...
        io
          ...
          .emit(NEvent.ServerOutgoing.EXPERIMENTAL_METRIX_PONG_OK, {
            message: `Experimental PONG (OK) for socket.id ${socket.id} in room ${getChannelName(incData.room)}`,
